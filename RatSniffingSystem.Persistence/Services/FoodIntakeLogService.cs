...
{
    public class FoodIntakeLogService : SessionLinkedServiceBase<FoodIntakeLog, FoodIntakeLogDto, CreateFoodIntakeLogDto, object>, IFoodIntakeLogService<FoodIntakeLogDto, CreateFoodIntakeLogDto>
    {
        public FoodIntakeLogService(AppDbContext context, IMapper mapper, ILogger<FoodIntakeLog> logger) : base(context, mapper, logger)
        {
        }
        ...
        public async Task<List<FoodIntakeLogDto>> GetByCcRangeAsync(double first, double last)
        {
            ...
            if (!foodIntakeLogs.Any())
            {
                ...
                throw new NotFoundException($"No food intake logs found in the range: {first} to {last} cc.");
            }
